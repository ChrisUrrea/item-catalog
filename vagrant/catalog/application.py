...
def gconnect():
    ...
    flash("You are now logged in! Hello %s!" % login_session['username'], 'success')
    return output
...
@app.route('/pawnshops/<int:pawnshop_id>/valuables/new/', methods=['GET', 'POST'])
def newValuable(pawnshop_id):
    ...
    if login_session['user_id'] != pawnshop.user_id:
        return "<script>function myFunction() {alert('You are not authorized to add valuables to this pawn shop. " \
               "Please create your own pawn shop in order to add valuable.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
def showValuables(pawnshop_id):
    ...
    valuables = session.query(Valuable).filter_by(pawnshop_id=pawnshop_id).all()
    return render_template('valuables.html', valuables=valuables, pawnshop=pawnshop, creator=creator)
...
@app.route('/pawnshops/<int:pawnshop_id>/valuables/<int:valuable_id>/edit', methods=['GET', 'POST'])
def editValuable(pawnshop_id, valuable_id):
    ...
    pawnshop = session.query(PawnShop).filter_by(id=pawnshop_id).one()
    if login_session['user_id'] != pawnshop.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit menu items to this pawnshop. " \
               "Please create your own pawnshop in order to edit items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
...
### DELETE ###
@app.route('/pawnshops/<int:pawnshop_id>/delete/', methods=['GET', 'POST'])
def deletePawnShop(pawnshop_id):
    "Delete a pawn shop with `pawnshop_id`"
    pawnshopToDelete = session.query(PawnShop).filter_by(id=pawnshop_id).one()
    if 'username' not in login_session:
        return redirect('/login')
    if pawnshopToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized to delete this pawnshop. Please create your own restaurant in order to delete.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        ...
        return render_template('deletepawnshop.html', pawnshop=pawnshopToDelete)

@app.route('/pawnshops/<int:pawnshop_id>/valuables/<int:valuable_id>/delete', methods=['GET', 'POST'])
def deleteValuable(pawnshop_id, valuable_id):
    "Delete a valuable with `valuable_id` from pawn shop `panwshop_id`"
    if 'username' not in login_session:
        return redirect('/login')
    pawnshop = session.query(PawnShop).filter_by(id=pawnshop_id).one()
    valuableToDelete = session.query(Valuable).filter_by(id=valuable_id).one()
    if login_session['user_id'] != pawnshop.user_id:
    ...
    if request.method == 'POST':
        ...
        return render_template('deletevaluable.html', valuable=valuableToDelete)
